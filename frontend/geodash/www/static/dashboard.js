...
function updateFlashBar(alerts) {
    ...
    if (redAlerts.length > 0) {
        ...
        // Group by type only — no area names
        const byType = {};
...
function updateCounterOverlay(alerts) {
    const overlay = document.getElementById('alert-counter-overlay');
...
function initNewsTicker() {
    // Respect saved preference — hidden by default
...
function toggleNewsTicker() {
    const ticker = document.getElementById('news-ticker');
    if (!ticker) return;
    const visible = ticker.style.display !== 'none';
    ticker.style.display = visible ? 'none' : 'flex';
